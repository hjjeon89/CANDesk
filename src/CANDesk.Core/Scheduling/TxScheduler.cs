...
public interface ITriggerCondition { bool IsSatisfied(in CanFrame receivedFrame); }
...
public sealed class TxScheduler(ICanDevice device) : ITxScheduler
{
    ...
    public Guid ScheduleCyclic(CanFrame templateFrame, TimeSpan period, PreSendModifier? preSendModifier = null)
    {
        ...
        var id = Guid.NewGuid(); var job = new Job(templateFrame, preSendModifier, period); _jobs[id] = job;
    ...
    {
        ArgumentNullException.ThrowIfNull(trigger); var id = Guid.NewGuid(); _jobs[id] = new Job(templateFrame, preSendModifier, trigger: trigger); return id;
    }
    ...
    {
        var received = frame;
        foreach (var job in _jobs.Values.Where(job => job.Trigger?.IsSatisfied(received) == true)) _ = SendJobAsync(job, _shutdown.Token);
    }
    public void Cancel(Guid jobId) { if (_jobs.TryRemove(jobId, out var job)) job.Cancellation?.Cancel(); }
    public void UpdatePayload(Guid jobId, ReadOnlySpan<byte> newData)
    {
        if (!_jobs.TryGetValue(jobId, out var job)) throw new KeyNotFoundException($"TX job '{jobId}' was not found.");
        lock (job.Gate) job.Frame = job.Frame.WithPayload(newData);
    }
    private async Task RunCyclicAsync(Guid id, Job job, CancellationToken ct)
    {
        using var timer = new PeriodicTimer(job.Period!.Value);
        try { while (await timer.WaitForNextTickAsync(ct).ConfigureAwait(false)) await SendJobAsync(job, ct).ConfigureAwait(false); }
        catch (OperationCanceledException) when (ct.IsCancellationRequested) { }
        finally { _jobs.TryRemove(id, out _); }
    }
    private async Task SendJobAsync(Job job, CancellationToken ct)
    {
        CanFrame frame; ulong count;
        lock (job.Gate) { frame = job.Frame; count = job.SendCount++; job.Modifier?.Invoke(frame.PayloadSpanWritable, count); }
        await device.SendAsync(frame, ct).ConfigureAwait(false);
    }
    public ValueTask DisposeAsync() { _shutdown.Cancel(); _shutdown.Dispose(); foreach (var id in _jobs.Keys) Cancel(id); return ValueTask.CompletedTask; }
}
